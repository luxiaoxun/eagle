...
public class LatencySink {
    ...
    public static SinkFunction<String> createLatencySink(ParameterTool parameter) throws IOException {
        ...
        String latencySink = parameter.get(Constant.LATENCY_SINK_TYPE);
        Type latencySinkType = Type.valueOf(latencySink.toUpperCase());

        switch (latencySinkType) {
            case KAFKA:
                String kafkaBootstrapServers = parameter.get(Constant.KAFKA_LATENCY_BOOTSTRAP_SERVERS);
                String latencyTopic = parameter.get(Constant.KAFKA_LATENCY_TOPIC);
