...
public class DynamicAlertFunction extends KeyedBroadcastProcessFunction<
    ...
    private MapStateDescriptor<Long, Set<Transaction>> windowStateDescriptor =
            new MapStateDescriptor<>("windowState", BasicTypeInfo.LONG_TYPE_INFO,
    ...
    public void processElement(Keyed<Transaction, String, Integer> value, ReadOnlyContext ctx, Collector<Alert> out)
        ...
        if (rule.getRuleState() == Rule.RuleState.ACTIVE) {
            ...
            ctx.output(Descriptors.demoSinkTag, "Rule " + rule.getRuleId() + " | " + value.getKey() + " : "
                    + aggregateResult.toString() + " -> " + ruleResult);
